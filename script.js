...
function updateMargin() {
    ...
    let screen = (currentSlide * slideItemWidth)
    document.querySelector('.slider--width').style.marginLeft = `-${screen}px`
}*/
    ...
    if(title2.length > 0) {
        let meuInterval = setInterval(()=>{
            ...
            if(title2.length === 0) {
                ...
                title2 = title1
                titlejunto = ''
                //c('.slider .slider--width .slider-principal .slider-principal--sub-title').style.display = 'block'
                setTimeout(()=>{
...
let num = 500
let res = atualwidth - num
for(let i = 3;i>= 0; i--) {
    all[i].addEventListener('click', (e)=>{
        let cloneImg = e.path[0].cloneNode()
